trigger BeneficiaryPersonTrigger on account (after update) {
    if(Trigger.isAfter && Trigger.isUpdate && !TruelinkStaticUtils.stopRecursionOnAccountUpdate){
        if(Trigger.new.size() == 1){
            ...
            if(acc.Update_All_Address_Fields__pc && apiSwitch.Update_Beneficiary_Address__c){
            	for( trust__c trust : [select id,TrueLink_ID__c,Truelink_Address_Id__c from trust__c where Beneficiary__c=:acc.PersonContactId and Trust_Status__c IN :allowedTrustStatus]){
            		if(String.isNotBlank(trust.Truelink_Address_Id__c) && acc.Update_All_Address_Fields__pc ){
                    		TrueLinkPartyTriggerHandler.updateAddress(acc.PersonContactId, trust.Truelink_Address_Id__c,acc.Update_All_Address_Fields__pc);
                    }
                }
            }
                    ...
                    for( trust__c trust : [select id,TrueLink_ID__c,Truelink_Address_Id__c from trust__c where Beneficiary__c=:acc.PersonContactId and Trust_Status__c IN :allowedTrustStatus]){
                        ...
                        if(isAddressChanged){
                            if(String.isNotBlank(trust.Truelink_Address_Id__c))
                                TrueLinkPartyTriggerHandler.updateAddress(acc.PersonContactId ,trust.Truelink_Address_Id__c, false);
                            else
